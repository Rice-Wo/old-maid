...
class blackjack(commands.Cog):
    ...
    async def bj(self, ctx):
        ...
        self.bj.game_start()
        self.bj_data['tot'] += 1
        input_data('blackjack', self.bj_data)
    ...
    async def bj_stats(self, ctx):
        tot = self.bj_data['tot']
        com = self.bj_data['com']
        player = self.bj_data['player']
        both = self.bj_data['both']

        player_win_rate = (player / tot)*100
        com_win_rate = (com / tot)*100
        both_rate = (both / tot)*100
        give_up_rate = ((tot-(player+com+both)) / tot)*100
